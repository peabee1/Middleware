...
// ---- Keyboard tap-cycle state (SPR-031 follow-up) -------------------------
//
// Mobile soft keyboards obscure the Submit button. New tap cycle:
//   tap 1:                focus, keyboard suppressed (auto-paste fires;
//                         user can submit without dismissing keyboard)
//   tap 2 within 2s:      keyboard shown (user wants to type)
//   tap 3 within 2s:      keyboard hidden, cycle resets
//   2s of no taps:        cycle resets — next tap is tap 1 again
//   real blur (tap out):  cycle resets immediately
//
// Mechanism: inputmode="none" suppresses the soft keyboard on focus. To
// toggle, switch the attribute and blur+refocus inside the user gesture.
// Only applied on touch-primary devices — desktop behaviour unchanged.

const KEYBOARD_TAP_WINDOW_MS = 2000;
const IS_TOUCH_PRIMARY =
  typeof window !== 'undefined' &&
  typeof window.matchMedia === 'function' &&
  window.matchMedia('(hover: none) and (pointer: coarse)').matches;

let tapState = 'off';            // 'off' | 'show' | 'hide'
let tapResetTimer = null;
let programmaticBlur = false;    // set true right before scripted $sql.blur()
let autoPasteCheckedThisFocus = false; // gate against double auto-paste on
                                       // blur+refocus inside the same focus
                                       // session
...
function clearAll() {
  ...
  lastSubmittedSql = '';
  // Reset keyboard tap cycle so next tap is treated as tap 1 again.
  resetTapState();
  if (IS_TOUCH_PRIMARY) {
    $sql.setAttribute('inputmode', 'none');
  }
}
...
// ---- Keyboard tap-cycle handlers (SPR-031 follow-up) ----------------------
...
function startTapResetTimer() {
  ...
  tapResetTimer = setTimeout(resetTapState, KEYBOARD_TAP_WINDOW_MS);
}

// Apply soft-keyboard visibility by toggling inputmode and refocusing.
// blur+focus must run synchronously inside the user gesture or the browser
// won't honor the keyboard change. Only programmatically blur if currently
// focused — otherwise the programmaticBlur flag would leak past a no-op.
function applyKeyboardState(show) {
  if (show) {
    $sql.removeAttribute('inputmode');
  } else {
    $sql.setAttribute('inputmode', 'none');
  }
  if (document.activeElement === $sql) {
...
function handleSqlPointerDown(e) {
  // Tap 1: focus with keyboard suppressed. Auto-paste fires from focus event.
  if (tapState === 'off') {
    e.preventDefault();
  ...
  // Tap 3: hide keyboard, reset cycle.
  if (tapState === 'hide') {
    e.preventDefault();
    applyKeyboardState(false);
    resetTapState();
...
function handleSqlBlur() {
  // Suppress the auto-paste flag reset for our own programmatic blur+focus.
  if (programmaticBlur) {
    programmaticBlur = false;
    return;
  }
  // Real blur — user tapped outside. Reset cycle and auto-paste gate so
  // the next tap into the input is treated as a fresh tap 1.
  resetTapState();
...
async function maybeAutoPaste() {
  // Per-focus-session gate: keyboard tap-cycle (tap 2/3) does blur+refocus
  // which fires focus a second time. We don't want a second clipboard read
  // mid-cycle. Reset on real blur in handleSqlBlur.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // SPR-031 follow-up: keyboard tap-cycle on touch-primary devices.
  // Desktop behaviour unchanged — no inputmode override, no pointerdown
  // intercept, just the standard focus + auto-paste flow above.
  if (IS_TOUCH_PRIMARY) {
    $sql.setAttribute('inputmode', 'none');
    $sql.addEventListener('pointerdown', handleSqlPointerDown);
  }
  // Blur listener wired regardless — needed to reset the auto-paste gate
  // on real blur even on desktop.
  ...
  if ($undoBtn) {
    $undoBtn.addEventListener('click', () => {
      ...
      // just undid. Real blur (genuine tap-out) will reset this flag.
      autoPasteCheckedThisFocus = true;
      $sql.focus();
    });
  }
