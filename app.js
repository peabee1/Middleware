...
//   CopyFAB.onPaste()              — host: input changed (paste/typed/glide)
...
(function () {
  ...
  // Position the FAB directly below the Submit FAB, anchored to its actual
  // measured rect. Robust against whatever CSS source positions the Submit
  // FAB. Re-runs on resize and (best-effort) when soft keyboard reflows.
  function reposition() {
  ...
  function bindUI() {
    ...
    if (submitEl) {
      ...
      // visualViewport.resize fires when soft keyboard appears/dismisses.
      if (window.visualViewport) {
        window.visualViewport.addEventListener('resize', reposition);
      }
      // One more reposition after fonts/styles settle.
...
function clearAll() {
  ...
  autoResize();
  // Reset Copy FAB to hidden — clipboard is no longer "the latest" because
  // there's no latest.
...
async function submit() {
  ...
  if (bailed) {
    ...
    // FAB state was already reset by clearAll → onResultsCleared.
    return;
  }
